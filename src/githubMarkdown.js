...
function createPRReportPart(prReport) {
    ...
    prReport.files.map((file)=> {
        let spoiler = renderCoverageDetails(file)
...
function renderCoverageDetails(file) {
    ...
    return "```diff\n" + addHash(dom.window.document.querySelector("pre").textContent) + "\n```"
}
...
function createCommentBody(fullReport, prReport){
    const header = "### :scroll: Test coverage report"
